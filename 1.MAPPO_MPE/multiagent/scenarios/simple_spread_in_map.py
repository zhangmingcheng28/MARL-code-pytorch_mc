...
# _map = cv2.imread('../my-maddpg-mpe-with-obstacle/map.jpg')
_map = cv2.imread('F:\githubClone\Multi_agent_AAC\simple_spread\my_environment\map.jpg')
sp = _map.shape
_map = cv2.resize(_map,(max(sp),max(sp)))
print(sp)

hsv_image = cv2.cvtColor(_map, cv2.COLOR_BGR2HSV)
...
red_points = np.transpose(np.nonzero(mask_red)).astype(np.int16)
white_points = np.transpose(np.nonzero(mask_white)).astype(np.int16)


# distances = cdist(white_points, red_points)
# mask = np.any(distances < agent_size, axis=1)
# filtered_white_points = white_points[~mask]
# white_points = filtered_white_points
# print((white_points))

red_points = (red_points/max(sp) - 0.5)*2

white_points = (white_points/max(sp) - 0.5)*2

print('red_points num: ', len(red_points))
print('white_points num: ',len(white_points))
...
class Scenario(BaseScenario):
    def make_world(self):
        ...
        设定世界中的障碍物位置
        '''
        world.obstacles = np.vstack([red_points[::10],obstacles])
        ...
        for i, agent in enumerate(world.agents):
            ...
            agent.collide = True
            agent.silent = True
            agent.size = agent_size  #改小飞机的size
            #agent.u_noise = agent_size
            '''
    ...
    def reset_world(self, world):
        ...
        for agent in world.agents:
            ...
            while True:
                agent.state.p_pos = np.array(random.choice(white_points))
            ...
            #agent.u_noise = agent.u_noise*0.99995

        for i, landmark in enumerate(world.landmarks):
            #landmark.state.p_pos = np.random.uniform(-1, +1, world.dim_p)
            landmark.state.p_pos = np.array(random.choice(white_points))
    ...
    def reward(self, agent, world):
        ...
        if dists<agent.size:
            ...
            rew -= (agent.size - np.clip(vel,0,agent.size)) * (1.0/agent.size)  # when agent velocity is less than agent size, we give a more negative reward (penalty), which is to discourage taking small steps. If agent's velocity is big enough, we don't give any penalty.
        # for l in world.landmarks:
        #     dists = [np.sqrt(np.sum(np.square(a.state.p_pos - l.state.p_pos))) for a in world.agents]
        #     rew -= min(dists)
        ...
        if agent.collide:
            for a in world.agents:
                if self.is_collision(a, agent):
                    rew -= 1
                    #agent.state.done = True  ##woc？自己撞自己也算？？
